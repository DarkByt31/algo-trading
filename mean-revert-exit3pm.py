...
def backtest(symbol, df):
    ...
    for i in range(1, len(df)):
        ...
        # Restrict trading between 9:30 AM and 3:00 PM
        if not (datetime.time(9, 30) <= row['datetime'].time() <= datetime.time(15, 0)):
            continue
    ...
    # Close all positions before 3:00 PM
    if position != 0:
        for i in reversed(range(len(df))):
            if df.iloc[i]['datetime'].time() <= datetime.time(15, 0):
